...
use crate::{gui::{Button, Component}, sync::UPSafeCell, syscall::PAD};
...
struct VirtIOINPUT(UPSafeCell<VirtIOInput<'static>>);
...
impl VirtIOINPUT {
    pub fn new(addr: usize) -> Self {
        Self(unsafe {
            UPSafeCell::new(VirtIOInput::new(&mut *(addr as *mut VirtIOHeader)).unwrap())
        })
    }
}
        ...
        match dtype {
            virtio_input_decoder::DecodeType::Key(key, r#type) => {
                ...
                if r#type == KeyType::Press {
                    let mut inner = PAD.try_exclusive_access().unwrap();
